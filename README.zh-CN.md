# fpverify — LLM API 与 Agent IDE 行为指纹验证器

检测一个 LLM 端点——API 中转站，或你付费的 agent IDE——背后运行的是否为它声称的模型。
...
再往上一层，同样的问题落在 agent IDE 头上：每个请求都经过厂商后端，实际应答的
是哪个 checkpoint，客户端看不见——悄悄改路由、暗中 A/B 都是如此（2026 年 4 月，
Cursor 3.0 被逆向发现内置了换皮的 Claude Code agent，连微调 checkpoint 一起
打包；厂商回应称是"不到 1% 流量的 A/B 测试"）。
...
### 场景一：你的 agent IDE 里跑的到底是什么模型？

适用于 Cursor、Codex 等能扇出 subagent 的 IDE。自带指纹库内置 9 个前沿模型的
harness 基线（2026-07，Cursor subagent，套卷协议）——据我们所知是唯一公开的
harness 内采集参考集。裸 API 指纹库回答不了这个问题：harness 内的分布是另一个
条件分布（厂商系统提示、采样参数、路由都在条件里），跨条件比对会误判
（实测记录：[`experiments/frontier/PROTOCOL.md`](experiments/frontier/PROTOCOL.md)）。

```bash
py -3.13 -X utf8 -m fpverify.cli library                          # 列出可用基线
py -3.13 -X utf8 -m fpverify.cli reproduce --claimed gpt-5.6-sol
```

把复核包里的 `cursor_prompt.md` 粘贴进 IDE，选好被测模型。它会扇出 N 个全新
subagent（无共享上下文），各自逐字回答原始十题套卷，然后按题汇总答案与众数。
读表方法（判读阈值随包附带）：

- 强题（参考占比 ≥ 90%）大多对上 → 与声称模型的 2026-07 基线一致；
- 多道强题同时偏离，或汇总结果与[实测表](#实测前沿模型无法随机)里**另一个**
  模型的列对上 → 行为不像声称的基线，这就是报警信号；
- 无论哪种结果，欢迎附汇总表开 issue——确认的模型版本更新后，基线会重新入册。

这是同协议比对（套卷对套卷），汇总表可直接对基线读数。它是对一份带日期快照的
变化检测，不是公证：入册那一刻信过一次平台的模型标签，此后抓的是悄悄变更与
跨模型错配。
...
（实测 gpt-5.6-sol：套卷抛硬币 = tails 11/11，官网冷启动单问 = heads 6/6）。
所以跑法按参考的出身选：
...
先列最近的邻居：三个工具与本项目共用同一篇底层论文的裸 API 指纹。论文参考数据集
本身是公开的（[Zenodo，CC-BY-4.0](https://doi.org/10.5281/zenodo.21278557)，
[tosea.ai 可在线浏览](https://tosea.ai/free-tools/fingerprints)）——如果你只需要
对着这份快照做一次裸 API 快检，用它们就够了。

| 工具 | 思路 | 判定类型 |
|---|---|---|
| [tosea.ai 指纹库 + 检测器](https://tosea.ai/free-tools/fingerprints) | 论文快照的网页化（167 模型，OpenRouter 裸 API、temp 1.0、单一采集日期），浏览器内采样你的端点 | 对快照做 JSD 比对 |
| [llm-fingerprint-detector](https://github.com/ToseaAI/llm-fingerprint-detector) | 零依赖 TypeScript CLI，内置同数据集的 11 个参考 | 固定样本 JSD 阈值 |
| [ChatHub 模型指纹](https://blog.chathub.gg/verify-ai-model-api-with-fingerprints/) | 商业网页工具：可信参考端点与被测端点对测（8 或 40 cell 套卷） | 固定样本 JSD 比对 |

思路不同的：

| 工具 | 思路 | 判定类型 |
|---|---|---|
| [LLMmap](https://github.com/pasquini-dario/LLMmap) | 主动指纹 + 训练分类器（52 模型；USENIX Security 2025） | 闭集分类 + 开集嵌入 |
...
| [cocodot llmprobe](https://github.com/cocodot2026/cocodot-llmprobe) | 六探针中转体检：自报身份、裁判模型打分、延迟、上下文、限流、一致性 | 打分卡；由中转商自营 |
...
2. 条件对齐的参考，含 agent-harness 渠道。指纹是（模型 × 渠道 × 协议）的条件
   分布；裸 API 快照回答不了"我的 agent IDE 里跑的是什么"，跨条件比对会误判。
   fpverify 把协议钉成一等属性，并内置 9 个前沿模型的 harness 基线（2026-07，
   原始数据全部提交、一条命令复现）——据我们所知是唯一公开的一份。
3. 良性漂移容差自动标定（Dirichlet 后验预测），不依赖手调阈值。
4. 可以脱离工具的判定：每个参考都能导出复核包，按参考自己的采集条件重跑证据，
   全程不经过本工具（见场景四）。
5. 对抗极限分析：明确给出检测失效区（随机稀释 ε ≈ 0.20~0.28，检出 ε 需 ~1/ε²
   样本），而非宣称检测不可战胜。
6. 立场中立：不卖商家"认证"、不接商家投放、判定本机计算无遥测——中转商自营的
   检测工具给不出这个保证。
...
- **对齐导入公开参考数据集**。底层论文的 167 模型裸 API 指纹以 CC-BY-4.0 公开在
  Zenodo。在探针措辞与采样参数能和我们对齐的前提下导入，可零采集成本扩充 `api`
  渠道覆盖；导入条目会明确标注出处与采集条件，跨协议比对照旧只降级为相对排名。
